...
> [!IMPORTANT]
> All client modules are considered stable and ready to be used in production.
...
```diff
dependencies {
+    smithyBuild("software.amazon.smithy.java:codegen-plugin:<replace with version>")
}
```
...
- `codegen:codegen-plugin` - Aggregate package that provides all code generation plugins.
...
- `aws-client-awsquery` - Implementation of [AWS Query](https://smithy.io/2.0/aws/protocols/aws-query-protocol.html#aws-query-protocol) protocol.

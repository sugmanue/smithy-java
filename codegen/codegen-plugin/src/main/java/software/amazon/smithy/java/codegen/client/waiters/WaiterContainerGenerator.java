 ...
 */
public final class WaiterContainerGenerator implements Consumer<CodeGenerationContext> {
        ...
        public void run() {
            ...
            for (var operation : waitableOperations) {
                ...
                for (var waiterEntry : trait.getWaiters().entrySet()) {
                    ...
                    // Inline waiter documentation
                    var waiterDoc = waiter.getDocumentation().orElse(null);
                    if (waiterDoc != null || waiter.isDeprecated()) {
                        writer.writeWithNoFormatting("/**");
                        if (waiterDoc != null) {
                            writer.writeWithNoFormatting(" * " + waiterDoc);
                        }
                        if (waiter.isDeprecated()) {
                            writer.writeWithNoFormatting(" * @deprecated Waiter is deprecated");
                        }
                        writer.writeWithNoFormatting(" */");
                    }

...
public final class ServiceGenerator implements
    ...
    public void accept(
        ...
        // In combined client+server mode, the directive symbol is the client symbol.
        // The server symbol is stored as a property.
        Symbol serverSymbol = directive.symbol()
                .getProperty(ServerSymbolProperties.SERVER_SERVICE_SYMBOL)
                .orElse(directive.symbol());
        var generatedSchemaIndex =
                serverSymbol.expectProperty(ServerSymbolProperties.TYPES_NAMESPACE) + ".GeneratedSchemaIndex";
        directive.context()
                .writerDelegator()
                .useFileWriter(
                        serverSymbol.getDeclarationFile(),
                        serverSymbol.getNamespace(),
                        writer -> {
                            ...
                            writer.putContext("service", serverSymbol);
